...
Changes:
    Added type of maze '--maze_type' as an additional command line parameter. This changes the end location if set as 'hilbert'
"""
...
def try_maze(max_moves, model, allowed_states, end_location):
    """
    ...
    Args:
        ...
        end_location (tuple): The end location of the maze
    Returns:
...
def main(cfg, maze_type):
    ...
    print(MAZE_CSV)
    height, width = len(maze), len(maze[0])
    if maze_type == 'hilbert':
        end_location = (0, width-1)
    else:
        end_location = (height - 1, width - 1)
    
    plot_maze(maze, MAZE_CSV, display_fig=False, save_fig=True, end_location=end_location)
    ...
    for iteration in range (cfg.max_episodes):
        ...
        if ((iteration+1) % test_interval == 0):
            num_moves, locations = try_maze(max_steps, model, allowed_states, end_location)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--maze_type", type=str, default=None, help="Indicate whether the input maze is a hilbert curve. Changes end location in maze.")
