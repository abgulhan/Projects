...
def wallpaper_main(args):
    ...
    if args.aug_train:
        train_transform = transforms.Compose([
            ...
            transforms.RandomAffine(degrees=(0, 360)),#), translate=(0.25,0.25)) # translate image by a max range of [-64,64] pixels and rotate between [0◦, 360◦]
            transforms.CenterCrop((181, 181)), # Assuming image size is 256
            transforms.RandomCrop((181-8, 181-8), padding=False), #equivalent to translate
    ...
    if args.train_size != None:
        ...
        class_size = len(train_dataset)//num_classes
        print(class_size)
        indices = []
        for i in range(num_classes):
            ind = np.random.choice(class_size, args.train_size//num_classes, replace=False) + class_size*i
            indices += list(ind)
        train_dataset = torch.utils.data.Subset(train_dataset, indices)
    
    if args.test_size != None:
        num_classes = len(test_dataset.classes)
        class_size = len(test_dataset)//num_classes
        indices = []
        for i in range(num_classes):
            ind = np.random.choice(class_size, args.test_size//num_classes, replace=False) + class_size*i
            indices += list(ind)
    ...
    if args.model == "CNN":
        ...
        print("using model CNN2")
        model = CNN2(input_channels=1, img_size=args.img_size, num_classes=num_classes).to(device)
    elif args.model == "CNN3":
        print("using model CNN3")
        model = CNN3(input_channels=1, img_size=args.img_size, num_classes=num_classes).to(device)
    else:
    ...
    if args.load_model != None:
        model.load_state_dict(torch.load(os.path.join(args.save_dir, 'Wallpaper', args.load_model)))
    ...
    if args.save_model:
        torch.save(model.state_dict(), os.path.join(args.save_dir, 'Wallpaper', "model_saved.pt"))
...
def taiji_main(args):
    ...
    if args.device == 'cuda':
        print(f"double check {torch.cuda.is_available()} ")
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    else:
        device = torch.device('cpu')
    ...
    for i in range(num_subs):
        ...
        if args.model == "MLP":
            ...
            model = MLP3(input_dim=train_data.data_dim, hidden_dim1=1024, hidden_dim2=1024, output_dim=num_forms).to(device)
        else:
            model = MLP(input_dim=train_data.data_dim, hidden_dim=1024, output_dim=num_forms).to(device)
